    ...
    {     
      ...
      match animator.current_animation.direction
      {
        Direction::Forward =>
        {
          ...
          if new_index < animator.current_animation.from || (new_index > animator.current_animation.to && animator.animation_queue.is_empty())
          {
            new_index = animator.current_animation.from;
          }
          else if new_index > animator.current_animation.to
          {
            new_index = play_next_animation_in_queue(animator.as_mut(),sprite.as_mut());
          }
        }

        Direction::Reverse =>
        {
          if new_index > animator.current_animation.to || (new_index <= animator.current_animation.from && animator.animation_queue.is_empty())
          {   
        ...
        {
          if (new_index > animator.current_animation.to && animator.current_animation.from +1 <= animator.current_animation.to) || (new_index <= animator.current_animation.from &&  animator.animation_queue.is_empty())
          {   
