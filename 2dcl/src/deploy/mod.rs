  ...
  T: AsRef<Path>,
{
  ...
  let mut chain = sign_ephemeral(&mut adapter,&ephemeral_identity,300).await?;
...
pub async fn prepare_deploy_data<T>(deploy_folder: T, server: &catalyst::Server) -> Result<(Vec<FileData>, EntityId)>
where
  T: AsRef<Path>{
  ...
  Ok(scene_deployer::build_entity_scene(pointers, files, &scene_files[0]) )
}

pub async fn sign_ephemeral(adapter: &mut EthereumAdapter, ephemeral_identity: &dcl_crypto::Account, duration_in_secs: u64) -> Result<Vec<AuthLink>> {
// Create AuthChain
