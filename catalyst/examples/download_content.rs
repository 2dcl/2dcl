...
async fn main() -> Result<()> {
  ...
  let scene_content = server.raw_get(format!("/content/contents/{}", scene_cid.0)).await?.text().await?;
  println!("{}", scene_content);

  // Entities
  let entities : Vec<String> = server.get(format!("/content/contents/{}/active-entities", scene_cid.0)).await?;
  println!("{:?}", entities);
  let scene_content : ContentResult = server.get(format!("/content/contents/{}", entities[0])).await?;
  ...
  for content in scene_content.content {
    ...
    println!("Downloading {} - ({}/{})", content.file, current, file_count);
