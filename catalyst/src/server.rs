...
impl Server {
  ...
  pub async fn raw_get<U>(&self, path: U) -> Result<reqwest::Response> where 
    U: AsRef<str> + std::fmt::Display,
  {
    Ok(self.http_client.get(format!("{}{}", self.base_url, path)).send().await?)
  }
...
mod tests {
  ...
  fn it_builds_with_base_url() {
      assert_eq!(Server::new("https://server.com").base_url, "https://server.com")
  }
  ...
  fn it_supports_custom_path_with_raw_get() {
    ...
    let response : reqwest::Response = tokio_test::block_on(server.raw_get("/lambdas/status")).unwrap();
